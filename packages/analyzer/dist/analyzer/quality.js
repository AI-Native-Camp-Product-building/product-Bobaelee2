 ...
 */
function scoreActionability(md, stats) {
    let score = 0;
    // A. 백틱 안의 실행 가능한 커맨드 (최대 40점)
    const inlineCommands = md.match(/`[^`]*(?:npm|yarn|pnpm|bun|uv|pip|cargo|make|go |docker|git )[^`]*`/gi) ?? [];
    const codeBlockCommands = md.match(/```[\s\S]*?(?:npm|yarn|pnpm|bun|uv|pip|cargo|make|go |docker|git )[\s\S]*?```/gi) ?? [];
    ...
    const pathWithRole = md.match(/[`\/](?:src|app|lib|components|pages|handlers?|services?|models?|schemas?|db|api|routes?|middleware|utils?|core)[\/\w]*[`]?\s*[→\-:—]\s*\S+/gi) ?? [];
    const pathOnly = md.match(/(?:^|\s)[`\/](?:src|app|lib|components|pages)[\/\w]*/gm) ?? [];
    const archPaths = Math.max(pathWithRole.length, pathOnly.length);
    if (pathWithRole.length >= 3)
        score += 25;
    else if (archPaths >= 5)
        score += 25;
    else if (archPaths >= 3)
    ...
    const hasVerifyLoop = /(?:반드시|항상|always|must)\s.*(?:실행|run|확인|check|verify|typecheck)/gi.test(md) ||
    ...
    if (hasEnvInfo && hasServiceInfo)
        score += 15;
    else if (hasEnvInfo || hasServiceInfo)
        score += 7;
 ...
 */
function scoreConciseness(md, stats, actionabilityScore) {
    let score = 0;
    ...
    if (/@[\.\~\/][^\s]+/m.test(md))
        score += 10; // @import 사용
    if (/\.claude\/rules/gi.test(md))
        score += 10; // rules/ 분리
    if (/CLAUDE\.local\.md/gi.test(md))
        score += 10; // local md 분리
 ...
 */
function scoreStructure(md, stats) {
    let score = 0;
    ...
    if (headings.some(h => standardNames.test(h)))
        score += 5;
    ...
    if (listRatio >= 0.2 && listRatio <= 0.6)
        score += 25;
    else if (listRatio >= 0.1 || listRatio <= 0.8)
        score += 15;
    else
        score += 5;
    // C. 우선순위 마킹 (최대 20점)
    const emphasisLines = lines.filter(l => /IMPORTANT|CRITICAL|MUST|NEVER|ALWAYS/i.test(l));
    const emphasisRatio = lines.length > 0 ? emphasisLines.length / lines.length : 0;
    if (emphasisLines.length > 0 && emphasisRatio <= 0.05)
        score += 20;
    else if (emphasisLines.length > 0 && emphasisRatio <= 0.10)
        score += 10;
    else if (emphasisLines.length > 0)
        score += 5;
    // D. 계층 구조 (최대 20점)
 ...
 */
function scoreUniqueness(md, stats) {
    let score = 0;
    // A. 프로젝트 고유 함정/주의사항 (최대 30점)
    const specificBans = md.match(/(?:절대|NEVER|금지|하지\s*마|DO\s*NOT)[\s\S]{0,30}?(?:[\w가-힣]{3,}\.[\w]+|\/[\w\/]{3,}|[\w가-힣]{3,}\s+(?:파일|모듈|폴더|디렉토리|서비스))/gi) ?? [];
    if (specificBans.length >= 3)
        score += 30;
    else if (specificBans.length >= 1)
        score += 15;
    // B. 워크플로우 규칙 (최대 25점)
    const hasWorkflow = /(?:branch|브랜치|PR|pull.request|merge|배포|deploy)[\s\S]{0,40}?(?:전략|strategy|규칙|rule|절차|process|flow)/gi.test(md) ||
        /(?:먼저|후에|전에|before|after|then)\s[\s\S]{0,30}?(?:확인|실행|리뷰|review|approve|승인)/gi.test(md);
    if (hasWorkflow)
        score += 25;
    // C. 환경 특이사항 (최대 25점)
    const constraints = md.match(/(?:캐싱용|only\s*(?:for|used)|전용|사용하지\s*않|대신\s*사용|instead\s*of|not\s*use|사용\s*금지|직접\s*(?:접근|호출)\s*금지)/gi) ?? [];
    ...
    if (toolContextCount >= 3)
        score += 20;
    else if (toolContextCount >= 1)
        score += 10;
    return Math.min(100, score);
}
/**
 ...
 * 위험한 실수를 방지하는 가드레일
 */
function scoreSafety(md, stats) {
    let score = 0;
    // A. 구체적 금지 규칙 (최대 35점)
    const bans = md.match(/(?:절대|NEVER|금지|하지\s*마|DO\s*NOT)[\s\S]{0,50}?\S{3,}/gi) ?? [];
    if (bans.length >= 6)
        score += 35;
        ...
        score += 15;
    // B. 민감 정보 보호 (최대 25점)
    const hasSensitiveWithBan = /(?:\.env|api.?key|secret|token|credential|password)[\s\S]{0,30}?(?:절대|NEVER|금지|하지\s*마|커밋|commit|노출|expose)/gi.test(md) ||
    ...
    if (hasSensitiveWithBan)
        score += 25;
    else if (hasSensitiveKeyword)
        score += 10;
    // C. 검증 의무화 (최대 25점)
    // ※ Actionability의 "검증 루프"와 의도적으로 중복 — 실행 가능성과 방어력 두 관점에서 모두 가치
    const hasTestGate = /(?:반드시|always|must)[\s\S]{0,30}?(?:test|테스트|typecheck|lint|check)/gi.test(md) ||
    ...
    if (stats.isExpandedInput) {
        if (stats.denyCount > 0)
            score += 5;
        if (stats.blocksDangerousOps)
            score += 5;
        if (stats.hookPromptCount > 0)
            score += 5;
    }
    return Math.min(100, score);
}
 ...
 */
export function calculateQualityScores(md, stats) {
