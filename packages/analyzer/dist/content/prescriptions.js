import { isNonDevProfile } from "../analyzer/scorer.js";
// ─── 우선순위 정렬 기준 ────────────────────────────────
const PRIORITY_ORDER = { high: 0, medium: 1, low: 2 };
// ─── 시그니처 처방전 (12개, 페르소나별 1개) ─────────────
const SIGNATURE_PRESCRIPTIONS = [
    {
        id: "sig-minimalist",
...
// ─── 차원별 처방전 (페르소나 특화 + 제네릭) ──────────────
const DIMENSIONAL_PRESCRIPTIONS = [
    // actionability — 페르소나 특화 (fortress)
    {
        ...
        tier: "dimensional",
        condition: (persona, _stats, quality) => persona === "fortress" && quality.actionability < 30,
    },
    // actionability — 제네릭
    {
    ...
    // conciseness — 길이 초과
    {
        ...
        tag: "dim:conciseness",
        tier: "dimensional",
        condition: (_persona, stats, quality) => quality.conciseness < 30 && stats.totalLines > 150,
    },
    // conciseness — 노이즈 (자율 에이전트 맥락이면 가드레일이므로 제외)
    {
        ...
        tier: "dimensional",
        condition: (_persona, _stats, quality, scores) => quality.conciseness < 40 && scores.agentOrchestration < 30,
    },
    ...
    // agentOrchestration — 높은 성숙도
    {
        ...
        tier: "dimensional",
        condition: (_persona, _stats, _quality, scores) => scores.agentOrchestration >= 60,
    },
    // agentOrchestration — 중간 (도입기)
    {
        ...
        tag: "dim:agentOrchestration",
        tier: "dimensional",
        condition: (_persona, _stats, _quality, scores) => scores.agentOrchestration >= 20 && scores.agentOrchestration < 60,
    },
    // structure — 페르소나 특화 (legislator)
    {
        ...
        tag: "dim:structure",
        tier: "dimensional",
        condition: (persona, _stats, quality) => persona === "legislator" && quality.structure < 30,
    },
    // structure — 제네릭
    {
    ...
    // safety — 페르소나 특화 (daredevil)
    {
        ...
        tag: "dim:safety",
        tier: "dimensional",
        condition: (persona, _stats, quality) => persona === "daredevil" && quality.safety < 20,
    },
    // safety — 제네릭
    {
    ...
    // teamImpact — 페르소나 특화 (puppet-master)
    {
        ...
        tag: "dim:teamImpact",
        tier: "dimensional",
        condition: (persona, _stats, _quality, scores) => persona === "puppet-master" && scores.teamImpact < 30,
    },
    // teamImpact — 제네릭
    {
        id: "dim-team-collab",
 ...
 * 부족하면 남은 차원별/공통에서 백필
 */
function selectPrescriptions(persona, stats, quality, scores) {
    // 조건 필터링
    const eligible = ALL_PRESCRIPTIONS.filter((p) => p.condition(persona, stats, quality, scores));
    // 티어별 분류 + 우선순위 정렬
...
export function generatePrescriptions(persona, mdStats, qualityScores, dimensionScores) {
    ...
    if (isNonDevProfile(mdStats, dimensionScores)) {
        for (const item of selected) {
            item.text = DEV_TO_GENERAL.reduce((t, [from, to]) => t.replace(from, to), item.text);
        }
    }
    return selected;
}
