 ...
 */
import type { PersonaKey, MdStats, StrengthItem } from "../types.js";
...
const STRENGTH_TEMPLATES: Record<PersonaKey, StrengthTemplate> = {
  ...
  polymath: (stats) => {
    ...
    return [
      {
        text: `7개 차원이 전부 고르게 높다는 건 이 테스트 안에서 가장 희귀한 시그니처예요. 대부분은 1~2개 차원에 몰빵하고 나머지가 비는데, 당신은 비어있는 데가 없어요.`,
      },
      {
        text: eco > 0
          ? `${eco}개 구성요소를 운용하면서도 특정 영역에 치우치지 않은 건 '균형을 유지하는 에너지'가 있다는 뜻이에요. 확장하면서 중심을 잃지 않는 건 훈련으로도 잘 안 되는 능력이에요.`
          : `자동화도 보안도 협업도 어느 하나 건드리기만 한 게 아니라 실제 운용 수준까지 올린 것 — 이게 팔방미인과 '어설프게 다 아는 사람'의 결정적 차이예요.`,
      },
      {
        text: "팀에 이런 사람이 한 명 있으면 전체 평균이 올라갑니다. 시그니처 기술이 없다는 게 약점처럼 보이지만, 사실 '특정 분야 전문가' 여러 명 모아놓은 것과 다른 종류의 가치예요 — 크로스 도메인 의사결정을 혼자서도 할 수 있는 유형.",
      },
    ];
  },
